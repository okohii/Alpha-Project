...
from app.core.presentation import execution_payload, parse_confirmation_candidate
...
class AvatarWSRenderer:
    """Adapter de saída do avatar para WebSocket."""
    def __init__(self, out: asyncio.Queue[dict[str, Any]]) -> None:
        self._out = out
    def show(self, command: AvatarCommand) -> None:
        put_dropping_oldest(self._out, {"type": "state", "state": command.state.value, "animation": command.animation, "expression": command.expression, "emotion": command.emotion, "idle_after_ms": command.idle_after_ms})
...
class AvatarSession:
    """Orquestra apenas transporte, estado e ciclo de vida da sessão."""
    ...
    async def receive_loop(self) -> None:
        while not self.cancel_event.is_set():
            ...
            if action == "start": await self._start_voice()
                ...
                self.cancel_event.set(); self._audio_abort.set(); self._interrupt_abort.set()
    ...
    async def _start_voice(self) -> None:
        if self._voice_task is not None and not self._voice_task.done():
            ...
            return
        self._audio_abort.clear(); self._interrupt_abort.clear()
    ...
    async def set_interaction(self, active: bool, reason: str = "") -> None:
        ...
        if active: await self.push({"type": "avatar_show", "animation": "wake", "duration_ms": 320})
            ...
            await self.push({"type": "execution_clear"}); await self.push({"type": "avatar_hide", "animation": "fade", "duration_ms": 420})


@router.websocket("/ws")
async def avatar_ws(websocket: WebSocket) -> None:
    ok, reason = validate_websocket_connection(websocket.headers.get("origin"), ws_client_host(websocket))
    if not ok:
        logger.warning("[avatar] ws rejeitado motivo=%s", reason)
        await websocket.close(code=1008); return
    await websocket.accept()
    session = AvatarSession(websocket)
    pump = asyncio.create_task(session.pump()); receive = asyncio.create_task(session.receive_loop())
    try: await asyncio.wait({receive, pump}, return_when=asyncio.FIRST_COMPLETED)
    except (WebSocketDisconnect, RuntimeError): pass
    finally:
        session.cancel_event.set(); session._audio_abort.set(); session._interrupt_abort.set()
...
def _resolve_ui_asset(asset_path: str) -> Path:
    ...
    if not normalized or normalized.startswith("/") or ".." in normalized.split("/"): return UI_DIR / "index.html"
    candidate = (UI_DIR / normalized).resolve()
    if UI_DIR.resolve() not in candidate.parents or not candidate.is_file(): return UI_DIR / "index.html"
    return candidate


@router.get("/ui/{asset_path:path}", include_in_schema=False)
async def avatar_asset(asset_path: str) -> FileResponse: return FileResponse(_resolve_ui_asset(asset_path))
