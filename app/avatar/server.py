...
_EXECUTION_EVENTS = {
    EventType.tool_started, EventType.tool_finished, EventType.tool_failed,
    EventType.skill_started, EventType.skill_finished,
    EventType.verification_started, EventType.verification_completed,
    EventType.task_step_completed, EventType.task_completed, EventType.task_failed,
    EventType.honesty_gate, EventType.textual_tool_call_blocked,
}
...
class AvatarWSRenderer:
    ...
    def show(self, command: AvatarCommand) -> None:
        self._out.put_nowait({
            "type": "state", "state": command.state.value, "animation": command.animation,
            "expression": command.expression, "emotion": command.emotion, "idle_after_ms": command.idle_after_ms,
        })
...
def _execution_payload(event: SystemEvent) -> dict[str, Any]:
    ...
    labels = {
        EventType.tool_started: "ferramenta executando", EventType.tool_finished: "ferramenta concluída",
        EventType.tool_failed: "ferramenta falhou", EventType.skill_started: "skill executando",
        EventType.skill_finished: "skill concluída", EventType.verification_started: "verificando",
        EventType.verification_completed: "verificação concluída", EventType.task_step_completed: "passo concluído",
        EventType.task_completed: "tarefa concluída", EventType.task_failed: "tarefa falhou",
        EventType.honesty_gate: "controle de evidência", EventType.textual_tool_call_blocked: "tool call bloqueada",
    }
    return {"type": "execution", "event": event.type.value, "label": labels.get(event.type, event.type.value),
            "target": str(tool or ""), "success": payload.get("success"), "duration_ms": event.duration_ms,
...
class AvatarSession:
    ...
    async def _run_voice(self) -> None:
        ...
        self._interaction = InteractionManager(
            ...
            timeout_seconds=settings.interaction_timeout_seconds,
            end_words=settings.interaction_end_words,
        )
        ...
        if not settings.wake_word_enabled:
            await self._set_interaction(True, reason="wake_word_disabled")
        else:
            await self._set_interaction(False, reason="waiting_for_wake_word")
        ...
        async with AsyncSessionLocal() as session:
            ...
            while not self.cancel_event.is_set():
                if self._interaction and self._interaction.expired():
                    ...
                    await self._set_interaction(False, reason="timeout")
                text = carry
                carry = None
                if text is None:
                    ...
                    try: path = await asyncio.to_thread(audio_io.record_microphone_vad, max_wait=60.0)
                        ...
                        await self.push({"type": "error", "message": f"microfone indisponível: {exc}"}); return
                ...
                if decision is None or not decision.accepted:
                    if decision and decision.ended:
                        ...
                        await self._set_interaction(False, reason="end_word")
                    continue
                if decision.activated:
                    await self._set_interaction(True, reason="wake_word")
                await self.push({"type": "caption", "from": "user", "text": text})
                ...
                try:
                    ...
                    await self.push({"type": "error", "message": f"falha no turno: {exc}"}); continue
                ...
                if response:
                    carry = await self._speak_interruptible(pipeline, response, emotion=self._emotion(answer))
    ...
    async def _speak_interruptible(self, pipeline: VoicePipeline, text: str, emotion: EmotionState | None) -> str | None:
        ...
        if result.get("status") != "ok":
            await self.push({"type": "error", "message": str(result.get("detail") or "tts indisponível")}); return None
        ...
        try: player, frame_rate, n_frames = await asyncio.to_thread(audio_io.play_wav_async, audio_path)
            ...
            await self.push({"type": "error", "message": f"áudio indisponível: {exc}"}); return None

        speech_started = asyncio.Event(); abort = threading.Event(); loop = asyncio.get_running_loop()
        recorder = asyncio.create_task(self._capture_onset(pipeline, speech_started, abort, loop))
        energy_task = asyncio.create_task(self._emit_speech_energy(audio_path, n_frames / frame_rate if frame_rate else 0.0))
        ...
        if speech_started.is_set():
            await asyncio.to_thread(audio_io.stop_wav_async, player); playback_done.cancel(); energy_task.cancel()
            try: return await recorder
            except Exception: return None
        started_wait.cancel(); abort.set(); energy_task.cancel()
        try: await recorder
        except Exception: pass
    ...
    async def _emit_speech_energy(self, audio_path: Path, duration: float) -> None:
        """Emit ~25fps RMS/peak values so the Astral Core can react like abstract lipsync."""
        try:
            with wave.open(str(audio_path), "rb") as wav:
                rate = wav.getframerate(); channels = wav.getnchannels(); width = wav.getsampwidth()
                if not rate or not width: return
    ...
    async def _capture_onset(self, pipeline: VoicePipeline, speech_started: asyncio.Event, abort: threading.Event, loop: asyncio.AbstractEventLoop) -> str | None:
        ...
        result = await pipeline.process(path)
        return (result.get("transcription") or "").strip() or None


@router.websocket("/ws")
async def avatar_ws(websocket: WebSocket) -> None:
    await websocket.accept(); logger.debug("[avatar] ws connected")
    session = AvatarSession(websocket); pump = asyncio.create_task(session.pump()); receive = asyncio.create_task(session.receive_loop())
