...
def _is_internal_content(text: str) -> bool:
    ...
    if "tool_calls" in payload: return True
    if "name" in payload and isinstance(payload["name"], str):
        if "arguments" in payload or set(payload) <= {"name", "arguments"}: return True
    return False
...
class VoicePipeline:
    ...
    def _bus(self) -> EventBus: return self.event_bus or EventBus()
    def _get_stt(self) -> FasterWhisperSTT:
        if self.stt is None: self.stt = FasterWhisperSTT()
        return self.stt
    def _get_tts(self) -> KokoroTTS:
        if self.tts is None: self.tts = KokoroTTS()
        return self.tts

    async def process(self, audio_path: Path) -> dict[str, Any]:
        event_bus = self._bus(); event_bus.emit(EventType.assistant_listening); await self.listener.start()
        try:
            event_bus.emit(EventType.assistant_transcribing); transcription = await self._get_stt().transcribe(audio_path)
            ...
            await self.listener.stop(); event_bus.emit(EventType.assistant_thinking)

    async def speak(self, text: str) -> dict[str, Any]: return await self.speak_expressive(text)

    async def speak_expressive(self, text: str, *, emotion: EmotionState | None = None) -> dict[str, Any]:
        try:
            settings = get_settings()
            if _is_internal_content(text):
                logger.warning("[tts] source=internal_content_blocked len=%d", len(text)); return {"status":"text_only","detail":"conteúdo interno bloqueado no TTS","text":text}
            prepared = clean_for_voice(text)
            if not prepared:
                logger.warning("[tts] empty_after_voice_clean source_len=%d", len(text or "")); return {"status":"text_only","detail":"texto sem conteúdo falável","text":text}
            logger.info("[tts] input_len=%d voice_len=%d text=%r", len(text or ""), len(prepared), prepared)
            if not settings.tts_emotion_enabled:
                state: EmotionState | None = EmotionState(); profile: DeliveryProfile | None = None
            else:
                state = emotion if emotion is not None else self.emotion_controller.resolve(prepared); profile = self.emotion_controller.build_profile(state)
            event_bus = self._bus()
            ...
            logger.exception("[tts] synthesis_failed"); return {"status":"text_only","detail":str(exc),"text":text}
