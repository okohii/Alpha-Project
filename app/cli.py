...
from app.database.models import Conversation, ManagedPathRecord
...
def _wake_words() -> list[str]:
    from app.speech.wakeword import _tokenize_words
...
async def _chat_wake_interactive(
    ...
    from app.speech.wakeword import strip_wake_word
...
async def _cmd_documents(args: argparse.Namespace) -> int:
    async with AsyncSessionLocal() as session:
        ...
        indexer = DocumentIndexer(
            file_manager,
            MemoryService(MemoryRepository(session), LocalEmbeddingProvider()),
        )
