...
from app.llm.vision import OllamaVisionError, OllamaVisionProvider
...
def test_provider_not_available_without_model(monkeypatch, tmp_path):
    monkeypatch.setattr("app.llm.vision.get_settings", lambda: _FakeSettings(""))
...
def test_describe_sends_image_and_returns_text(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("app.llm.vision.get_settings", lambda: _FakeSettings("gemma3:4b"))
    ...
    captured = {}
    monkeypatch.setattr("app.llm.vision.httpx.AsyncClient", FakeClient)
