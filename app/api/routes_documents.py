...
async def _build_indexer(session) -> DocumentIndexer:
    ...
    if allowed_paths:
        file_manager.allowed_directories = [Path(path).expanduser().resolve() for path in allowed_paths]
    return DocumentIndexer(file_manager=file_manager, repository=DocumentRepository(session), embedding_provider=LocalEmbeddingProvider())
...
@router.get("")
async def list_documents(session=Depends(get_session), _auth=_EXEC) -> list[dict]:
    return await DocumentRepository(session).list_documents()


@router.post("/search")
async def search_documents(payload: DocumentSearchRequest, session=Depends(get_session), _auth=_EXEC) -> dict:
    indexer = await _build_indexer(session)
