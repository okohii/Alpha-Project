...
Item {
    ...
    readonly property real statePulse: state === "speaking" ? 0.18 + energy * 0.34 :
        state === "listening" ? 0.13 + energy * 0.22 :
    ...
    View3D {
        ...
        DirectionalLight { eulerRotation.x: -35; eulerRotation.y: 25; brightness: 2.2 + root.statePulse * 3.8; color: root.accent }
        PointLight { position: Qt.vector3d(0, 80, 260); brightness: 2.8 + root.energy * 7 + root.peak * 3; color: root.accent }
        ...
        Node {
            ...
            Model { source: "#Sphere"; scale: Qt.vector3d(1.55,1.55,1.55); materials: PrincipledMaterial { baseColor: root.accent; metalness: 0.45; roughness: 0.16; emissiveFactor: Qt.vector3d(root.accent.r*0.72,root.accent.g*0.72,root.accent.b*0.72); opacity: 1.0 } }
            Model { source: "#Sphere"; scale: Qt.vector3d(1.72,1.72,1.72); materials: PrincipledMaterial { baseColor: root.accent; metalness: 0.25; roughness: 0.25; transmissionFactor: 0.28; opacity: 0.18 + root.energy*0.20 + root.statePulse*0.22; emissiveFactor: Qt.vector3d(root.accent.r*0.18,root.accent.g*0.18,root.accent.b*0.18) } }
        }

        Node { id: ringNodeA; eulerRotation.x: 65; eulerRotation.y: root.motion*34 + root.energy*20; Model { source:"#Cylinder"; scale:Qt.vector3d(2.05 + root.energy*0.3,0.035,2.05 + root.energy*0.3); materials:PrincipledMaterial { baseColor:root.accent; metalness:0.8; roughness:0.15; emissiveFactor:Qt.vector3d(root.accent.r*0.18,root.accent.g*0.18,root.accent.b*0.18); opacity:0.88 } } }
        Node { id: ringNodeB; eulerRotation.x:-55; eulerRotation.z:30; eulerRotation.y:-root.motion*25; Model { source:"#Cylinder"; scale:Qt.vector3d(2.45 + root.peak*0.5,0.028,2.45 + root.peak*0.5); materials:PrincipledMaterial { baseColor:root.accent; metalness:0.7; roughness:0.2; emissiveFactor:Qt.vector3d(root.accent.r*0.20,root.accent.g*0.20,root.accent.b*0.20); opacity:0.62 } } }
        Node { id: ringNodeC; eulerRotation.y:90; eulerRotation.z:root.motion*18; Model { source:"#Cylinder"; scale:Qt.vector3d(2.95 + root.energy*0.7,0.022,2.95 + root.energy*0.7); materials:PrincipledMaterial { baseColor:root.accent; metalness:0.65; roughness:0.22; emissiveFactor:Qt.vector3d(root.accent.r*0.10,root.accent.g*0.10,root.accent.b*0.10); opacity:0.34 + root.statePulse + root.energy*0.18 } } }
        Model { source:"#Sphere"; scale:Qt.vector3d(2.5 + root.energy*0.6,2.5 + root.energy*0.6,2.5 + root.energy*0.6); materials:PrincipledMaterial { baseColor:root.accent; opacity:0.035 + root.energy*0.045 + root.statePulse*0.05; transmissionFactor:0.8; emissiveFactor:Qt.vector3d(root.accent.r*0.04,root.accent.g*0.04,root.accent.b*0.04) } }
    }
