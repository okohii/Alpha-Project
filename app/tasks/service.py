...
from app.database.models import ManagedPathRecord, TaskRecord
...
class TaskExecutorService:
    ...
    async def register_allowed_path(self, path: str, entry_type: str = "directory", source: str = "task_executor") -> ManagedPathRecord:
        ...
        record = ManagedPathRecord(
            ...
            is_allowed=1,
            source=source,
            metadata_={"registered_at": datetime.now(timezone.utc).isoformat()},
    ...
    async def _run_action(self, action: str, params: dict[str, Any]) -> dict[str, Any]:
        if action == "create_file":
            return self.file_manager.create_file(str(params["path"]), str(params.get("content", "")))
