...
async def test_task_executor_registers_allowed_path_and_creates_task(tmp_path: Path):
    ...
    saved_path = await service.register_allowed_path(str(tmp_path / "workspace"), entry_type="directory")
    task = await service.create_task(
        title="Criar helper",
        instruction="Crie um método auxiliar para somar números.",
        action="create_method",
        params={"file_path": str(tmp_path / "helper.py"), "method_name": "sum_values", "method_body": "return sum(values)"},
    )
...
async def test_task_executor_creates_method_in_allowed_file(tmp_path: Path):
    ...
    service = TaskExecutorService(FakeTaskRepository(), FakeManagedPathRepository(), FileManager([tmp_path]))
    task = await service.create_task(
        title="Adicionar utilitário",
        instruction="Adicione um método de soma.",
        action="create_method",
        params={"file_path": str(file_path), "method_name": "sum_values", "method_body": "return sum(values)"},
    )
