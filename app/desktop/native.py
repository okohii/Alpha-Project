...
class _WebSocketThread(threading.Thread):
    def __init__(self, url: str, incoming: Queue[dict[str, Any]]) -> None:
        super().__init__(name="alpha-native-ws", daemon=True); self.url=url; self.incoming=incoming; self._outgoing:Queue[dict[str,Any]]=Queue(); self._stop_event=threading.Event(); self._ws:Any=None
...
class NativeAlphaWindow(QWidget):
    ...
    def _update_avatar_hud(self,state:str,activity:str)->None:
        ...
        state=state if state in _STATE_LABELS else "idle";color=_STATE_COLORS[state];self.status_overlay.setText(f"{_STATE_ICONS[state]}  {_STATE_LABELS[state]}");self.status_overlay.setStyleSheet(f"QLabel {{ background: transparent; border: none; color: {color}; font-size: 12px; font-weight: bold; padding: 2px; }}");self.activity.setText(activity);self.activity.setStyleSheet("QLabel { background: transparent; border: none; color: #e8f7ff; font-size: 10px; padding: 2px; }")
    ...
    def _drain_events(self)->None:
        ...
        while processed<40:
            ...
            if kind=="state":self._set_state(str(message.get("state","idle")))
                ...
                else:self._update_avatar_hud("success","Tarefa concluída");QTimer.singleShot(1800,lambda:self._set_state("idle"))
            elif kind=="error":
                self._set_state("error")
                if self.mode=="chat":self.activity.setText(str(message.get("message","erro")))
                else:self._update_avatar_hud("error",str(message.get("message","erro"))[:180])
            elif kind=="connected":
                if self.mode=="chat":self.activity.setText("Conectado ao ALPHA Core")
                else:self._send({"action":"start"})
...
def run_native(*,mode:str,host:str,port:int,width:int,height:int)->int:
    server,backend_thread=start_backend(host,port);ws_path="avatar" if mode=="avatar" else "overlay";ws_url=f"ws://{host}:{port}/{ws_path}/ws";app=QApplication.instance() or QApplication([]);app.setApplicationName("ALPHA");window=NativeAlphaWindow(mode=mode,ws_url=ws_url,width=width,height=height);window.show()
