...
def test_type_text_tool_types_into_focused_window(monkeypatch):
    ...
    monkeypatch.setattr(keyboard_module, "type_text", fake_type)
    tool = TypeTextTool(ApplicationLauncher())
...
def test_type_text_tool_activates_app_window(monkeypatch):
    monkeypatch.setattr(keyboard_module, "type_text", lambda text: len(text))
    monkeypatch.setattr(keyboard_module, "activate_window", lambda name: True)
    tool = TypeTextTool(ApplicationLauncher())
