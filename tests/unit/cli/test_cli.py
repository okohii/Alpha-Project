...
def _patch_cli(monkeypatch) -> None:
    monkeypatch.setattr("app.cli.app.initialize_database", _noop_init)
    monkeypatch.setattr("app.cli.app.AsyncSessionLocal", FakeSessionFactory())


def _patch_voice(monkeypatch) -> FakeVoicePipeline:
    fake_pipeline = FakeVoicePipeline()
    monkeypatch.setattr("app.cli.app.VoicePipeline", lambda *a, **k: fake_pipeline)
    monkeypatch.setattr("app.cli.app.audio_io.play_wav", mock.Mock())
...
def test_main_without_command_starts_interactive_chat(monkeypatch):
    ...
    monkeypatch.setattr("app.cli.app._cmd_chat", fake_cmd_chat)
...
def test_cli_chat_single_message(monkeypatch, capsys):
    ...
    _patch_cli(monkeypatch)
    monkeypatch.setattr("app.cli.app.build_agent", _fake_build_agent)
...
def test_cli_terminal_slash_commands_do_not_reach_llm(monkeypatch, capsys):
    from app.cli.app import _chat_terminal
    from app.cli.renderer import TerminalRenderer

    _patch_cli(monkeypatch)
    monkeypatch.setattr("app.cli.app.build_agent", _fake_build_agent)
    monkeypatch.setattr("app.cli.app._start_scheduler_or_none", _noop_init)
    prompts = iter(["/help", "/verbosity debug", "/exit"])
    ...
    monkeypatch.setattr(TerminalRenderer, "prompt_user", fake_prompt)

    code = asyncio.run(_chat_terminal(None, as_json=False))
...
def test_cli_chat_voice_fluid_loop_hears_and_replies(monkeypatch, capsys):
    ...
    monkeypatch.setattr("app.cli.app.VoicePipeline", lambda: pipeline)
    monkeypatch.setattr("app.cli.app.audio_io.record_microphone_vad", recorder)
    play_wav_async = mock.Mock(return_value=("winsound", 16000, 160000))
    stop_wav_async = mock.Mock()
    monkeypatch.setattr("app.cli.app.audio_io.play_wav_async", play_wav_async)
    monkeypatch.setattr("app.cli.app.audio_io.stop_wav_async", stop_wav_async)
...
def test_cli_chat_voice_exits_on_spoken_word(monkeypatch, capsys):
    ...
    play_wav = mock.Mock()
    monkeypatch.setattr("app.cli.app.VoicePipeline", lambda: pipeline)
    monkeypatch.setattr("app.cli.app.audio_io.play_wav", play_wav)
...
def test_cli_memories_list_default(monkeypatch, capsys):
    ...
    _patch_cli(monkeypatch)
    monkeypatch.setattr("app.cli.app.MemoryService", FakeMemoryService)
...
def test_cli_settings_with_json(monkeypatch, capsys):
    ...
    monkeypatch.setattr("app.cli.app.ManagedPathRepository", mock.Mock(return_value=patched_repo))
...
def test_cli_tasks_list_default(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "app.cli.app._make_task_service",
...
def test_cli_listen_transcribes_microphone(monkeypatch, capsys):
    ...
    _patch_cli(monkeypatch)
    monkeypatch.setattr(
        "app.cli.app.audio_io.record_microphone",
        mock.Mock(return_value=Path("C:/tmp/fake-recording.wav")),
    )
    fake_stt = mock.Mock()
    fake_stt.transcribe = mock.AsyncMock(return_value=FakeTranscription())
    monkeypatch.setattr("app.cli.app.FasterWhisperSTT", mock.Mock(return_value=fake_stt))
