...
from app.core.presentation import confirmation_payload, execution_payload
from app.db.session import AsyncSessionLocal
from app.overlay.state import TERMINAL_EVENTS, TEXT_EVENTS, TOOL_EVENTS, OverlayState, state_for_event
...
_EXECUTION_EVENTS = {EventType.tool_selected, EventType.tool_started, EventType.tool_finished, EventType.tool_failed, EventType.skill_started, EventType.skill_finished, EventType.verification_started, EventType.verification_completed, EventType.task_created, EventType.task_step_completed, EventType.task_completed, EventType.task_failed, EventType.honesty_gate, EventType.textual_tool_call_blocked}


def _serialize_event(event: SystemEvent) -> dict[str, Any]:
    return {"event": event.type.value, "payload": dict(event.payload or {}), "duration_ms": event.duration_ms}


def _execution_payload(event: SystemEvent) -> dict[str, Any]:
    return execution_payload(event)
...
def _text_payload(event: SystemEvent) -> dict[str, Any]:
    ...
    return {"type": "caption", "from": "user" if event.type is EventType.user_message else "assistant", "text": str(text), "event": event.type.value}


def _parse_confirmation_candidate(candidate: str) -> dict[str, str]:
    """Compatibilidade para consumidores antigos; regra fica em core.presentation."""
    return confirmation_payload(candidate)


class _OverlaySession:
    """Sessão de UI: recebe eventos e os projeta no estado do overlay."""
    ...
    async def _pump_events(self) -> None:
        while True:
            ...
            if event.type in _EXECUTION_EVENTS:
                await self.send(execution_payload(event))
            if event.type in TOOL_EVENTS or (event.type in TEXT_EVENTS and event.type is not EventType.user_message):
                ...
                await self.send({"type": "done", "event": event.type.value, "conversation_id": cid})

    async def permission_request(self, candidate: str) -> bool:
        await self.send(confirmation_payload(candidate))
        try:
...
async def _handle_chat(session: Any, overlay: _OverlaySession, message: str, stream: bool) -> None:
    agent = await build_agent(session, permission_prompt=overlay.permission_request, event_bus=overlay.event_bus, cancel_event=overlay.cancel_event)
    if stream:
...
async def _handle_voice(overlay: _OverlaySession, raw: dict[str, Any]) -> None:
    ...
    try:
        ...
        await overlay.send({"type": "transcription", "text": result.get("transcription", ""), "language": result.get("language", "")})
...
async def overlay_ws(websocket: WebSocket) -> None:
    ...
    if not ok:
        logger.warning("[overlay] ws rejeitado origin=%r host=%r motivo=%s", origin, client_host, reason)
                ...
                if action in ("chat", "chat_stream"):
                    ...
                    if overlay.chat_task is not None and not overlay.chat_task.done():
                        ...
                        try:
                            ...
                            pass
                    overlay.chat_task = asyncio.create_task(_handle_chat(session, overlay, message, action == "chat_stream"))
                elif action == "cancel":
                    overlay.cancel_event.set()
                elif action == "confirm":
