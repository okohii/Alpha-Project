...
from app.tools import gui as gui_module
...
def test_mouse_click_moves_then_clicks(monkeypatch):
    events = []
    monkeypatch.setattr(gui_module, "mouse_move", lambda x, y: events.append(("move", x, y)))
    monkeypatch.setattr(
        gui_module,
        "_native_mouse_event",
...
def test_press_key_tool_result(monkeypatch):
    monkeypatch.setattr(gui_module, "press_sequence", lambda key: 1)
    tool = PressKeyTool()
...
def test_mouse_scroll_tool(monkeypatch):
    called = {}
    monkeypatch.setattr(gui_module, "mouse_scroll", lambda delta: called.update(delta=delta))
...
def test_screenshot_tool_writes_inside_allowed_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gui_module, "capture_screen", lambda path: path)
